                ...
                components={{
                    code({ node, className, children, ...props }) {
                        ...
                        return !isInline ? (
                            ...
                            <code className="bg-gray-100 text-sm px-1 py-0.5 rounded"{...props}>{children}</code>
                        );
                    },
                    p({ children }) {
                    ...
                    blockquote({ children }) {
                        return <blockquote className="border-l-4 border-gray-200 pl-4 italic my-2">{children}</blockquote>;
                    },
                    h1({ children }) { 
                    ...
                    h4({ children }) {
                        return <h4 className="text-base font-bold mb-2 mt-4 ">{children}</h4>;
                    },
                    a({ href, children }) {
                    ...
                    th({ children }) {
                        return <th className="uppercase tracking-wide px-3 py-2 text-gray-500 text-xs font-medium text-left">{children}</th>;
                    },
                    td({ children }) {
                        return <td className="text-sm whitespace-nowrap px-3 py-2">{children}</td>;
                    },
                    hr() {
...
function CodeBlock({ code, language }) {
    ...
    return <div className='relative my-6 rounded-lg overflow-hidden border border-gray-200 bg-gray-50'>
        <div className='flex justify-between items-center px-4 py-2 bg-gray-100 border-b border-gray-200'>
            <div className='flex items-center space-x-2'>
                ...
                <span className='text-xs text-gray-600 tracking-wide font-semibold uppercase'>
                    {language || 'Code'}
                </span>
            </div>
            <button onClick={copyCode} className='text-gray-500 hover:text-gray-700 focus:outline-none relative group' aria-label="Copy code to clipboard">
                {copied ? (
